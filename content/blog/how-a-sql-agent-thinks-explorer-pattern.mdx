---
title: How a SQL Agent Thinks: Walking Through the Explorer Pattern
description: Most text-to-SQL demos show the happy path. This post walks through the real explorer-first workflow an SQL agent uses to disambiguate schema, resolve filters, and enforce security before execution.
...
In my [previous post](https://tomvsaji.com), I covered a year of building a SQL agent on top of a complex, legacy on-premises database — what worked, what hurt, and how the architecture evolved. That post covered the **why**. This one covers the **how**.
...
| column      | type    | sample     |
| ----------- | ------- | ---------- |
| employee_id | VARCHAR | EMP001     |
| department  | VARCHAR | D042       |
| dept_name   | VARCHAR | Operations |
...
| ------------ | ------- | ---------- |
| employee_id  | VARCHAR | EMP001     |
| absence_date | DATE    | 2025-11-03 |
| reason_code  | VARCHAR | NP         |
| approved     | BIT     | 0          |
