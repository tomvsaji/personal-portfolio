...
- **Get Distinct Values** — returns unique values for a column with optional search. The agent then uses iterative tool-calling (varied lookups and disambiguation prompts) to resolve misspellings and near-matches without guessing.
