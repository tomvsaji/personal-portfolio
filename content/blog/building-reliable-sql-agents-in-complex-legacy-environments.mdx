---
title: Building Reliable SQL Agents in Complex On-Prem SQL Environments
description: Lessons from building and hardening an enterprise text-to-SQL agent in a complex on-prem SQL Server environment with linked-server dependencies.
...
The database was a complex on-premises SQL Server environment with linked-server dependencies spanning multiple business domains. Behind-the-scenes coupling made it fragile: joins failed unexpectedly, performance degraded on complex queries, and there was no fuzzy search or text indexing to handle real-world inputs like misspelled names or partial matches.
